...
public class PostReach
{
	...
	public static PostReach FromJson(VkResponse response)
    {
        ...
        {
            ReachSubscribers = response["reach_subscribers"], ReachTotal = response["reach_total"],
            Links = response["links"], ToGroup = response["to_group"], JoinGroup = response["join_group"],
            Report = response["report"], Hide = response["hide"], Unsubscribe = response["unsubscribe"]
        };
