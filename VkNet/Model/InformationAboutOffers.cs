...
public class InformationAboutOffers
{
	...
	public static InformationAboutOffers FromJson(VkResponse response)
    {
        ...
        {
            Id = response["id"], Title = response["title"], Instruction = response["instruction"],
            InstructionHtml = response["instruction_html"], ShortDescription = response["short_description"],
            Description = response["description"], Img = new Uri(response["img"]), Tag = response["tag"],
