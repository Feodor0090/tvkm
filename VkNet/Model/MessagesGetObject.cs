...
public class MessagesGetObject
{
	...
	public static MessagesGetObject FromJson(VkResponse response)
    {
        ...
        {
            TotalCount = response["count"], Unread = response["unread"] ?? response["unread_dialogs"],
            RealOffset = response["real_offset"], Messages = response["items"].ToReadOnlyCollectionOf<Message>(m => m),
            InRead = response["in_read"], OutRead = response["out_read"]
        };
