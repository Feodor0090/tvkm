...
public class Counters
{
    ...
    public static Counters FromJson(VkResponse response)
    {
        ...
        {
            Albums = response["albums"], Videos = response["videos"], Audios = response["audios"],
            Photos = response["photos"], Notes = response["notes"], Friends = response["friends"],
            Groups = response["groups"], OnlineFriends = response["online_friends"],
            MutualFriends = response["mutual_friends"], UserVideos = response["user_videos"],
            Followers = response["followers"], UserPhotos = response["user_photos"],
            Subscriptions = response["subscriptions"], TopicsCount = response["topics"],
            DocumentsCount = response["docs"], Pages = response["pages"], Messages = response["messages"],
            Gifts = response["gifts"], Events = response["events"], Notifications = response["notifications"],
            Sdk = response["sdk"], AppRequests = response["app_requests"]
        };
