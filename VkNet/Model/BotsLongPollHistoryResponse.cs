...
public class BotsLongPollHistoryResponse
{
	...
	public static BotsLongPollHistoryResponse FromJson(VkResponse response)
    {
        if (response.ContainsKey("failed"))
        {
            ...
            if (code == LongPollException.OutdateException) throw new LongPollOutdateException(response["ts"]);

            if (code == LongPollException.KeyExpiredException) throw new LongPollKeyExpiredException();

            if (code == LongPollException.InfoLostException) throw new LongPollInfoLostException();
        }
        ...
        VkResponseArray updates = response["updates"];
        var updateList = new List<GroupUpdate.GroupUpdate>();

        foreach (var update in updates) updateList.Add(GroupUpdate.GroupUpdate.FromJson(update));
