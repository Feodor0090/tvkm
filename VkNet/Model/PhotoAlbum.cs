...
public class PhotoAlbum
{
    ...
    public static PhotoAlbum FromJson(VkResponse response)
    {
        ...
        return new PhotoAlbum
        {
            ...
            OwnerId = Utilities.GetNullableLongId(response["owner_id"]), Title = response["title"],
            Description = response["description"], Created = response["created"], Updated = response["updated"],
            Size = response["size"], PrivacyView = privacy.ToReadOnlyCollectionOf<Privacy>(x => x),
            PrivacyComment = privacyComment.ToReadOnlyCollectionOf<Privacy>(x => x), CanUpload = response["can_upload"],
            ThumbSrc = response["thumb_src"], Sizes = response["sizes"].ToReadOnlyCollectionOf<PhotoSize>(x => x),
            CommentsDisabled = response["comments_disabled"], UploadByAdminsOnly = response["upload_by_admins_only"],
