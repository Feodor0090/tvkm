...
public class StandInLife
{
    ...
    public static StandInLife FromJson(VkResponse response)
    {
        ...
        {
            Political = response["political"], Languages = response["langs"].ToReadOnlyCollectionOf<string>(x => x),
            Religion = response["religion"], InspiredBy = response["inspired_by"], PeopleMain = response["people_main"],
            LifeMain = response["life_main"], Smoking = response["smoking"], Alcohol = response["alcohol"]
        };
