...
public class VideoCatalog
{
	...
	public static VideoCatalog FromJson(VkResponse response)
    {
        ...
        {
            Id = response["id"], Name = response["name"], CanHide = response["can_hide"], Type = response["type"],
            Next = response["next"], Items = response["items"].ToReadOnlyCollectionOf<VideoCatalogItem>(x => x),
            View = response["view"]
        };
