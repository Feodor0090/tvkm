...
public class PostDonut
{
	...
	public static PostDonut FromJson(VkResponse response)
    {
        ...
        res.IsDonut = response["is_donut"];
        res.PaidDuration = response["paid_duration"];
        res.Placeholder = response["placeholder"];
        res.CanPublishFreeCopy = response["can_publish_free_copy"];
        res.EditMode = response["edit_mode"];
