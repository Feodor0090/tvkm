...
public class Career
{
	...
	public static Career FromJson(VkResponse response)
    {
        ...
        {
            GroupId = response["group_id"], Company = response["company"], CountryId = response["country_id"],
            CityId = response["city_id"], CityName = response["city_name"], From = response["from"],
            Until = response["until"], Position = response["position"]
        };
