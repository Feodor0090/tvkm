...
public class VideoCatalogItem
{
	...
	public static VideoCatalogItem FromJson(VkResponse response)
    {
        ...
        {
            Id = response["id"], OwnerId = response["owner_id"], Title = response["title"], Type = response["type"],
            Duration = response["duration"], Description = response["description"], Date = response["date"],
            Views = response["views"], Comments = response["comments"], Photo130 = response["photo_130"],
            Photo320 = response["photo_320"], Photo640 = response["photo_640"], CanAdd = response["can_add"],
            CanEdit = response["can_edit"], Count = response["count"], Photo160 = response["photo_160"],
            UpdatedTime = response["updated_time"], IsPrivate = response["is_private"],
            AddingDate = response["adding_date"], Photo800 = response["photo_800"]
        };
