...
public class School
{
    ...
    public static School FromJson(VkResponse response)
    {
        var school = new School
        {
            ...
            City = Utilities.GetNullableLongId(response["city"]), Name = response["name"] ?? response["title"],
            YearFrom = response["year_from"], YearTo = response["year_to"], YearGraduated = response["year_graduated"],
            Class = response["class"], Speciality = response["speciality"], Type = response["type"],
