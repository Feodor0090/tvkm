...
public class University
{
    ...
    public static University FromJson(VkResponse response)
    {
        ...
        {
            Id = response["id"], Country = response["country"], City = response["city"],
            Name = response["name"] ?? response["title"], Faculty = response["faculty"],
            FacultyName = response["faculty_name"], Chair = response["chair"], ChairName = response["chair_name"],
            Graduation = response["graduation"],

            // установлено экcпериментальным путем
            EducationForm = response["education_form"], EducationStatus = response["education_status"]
        };
    }
