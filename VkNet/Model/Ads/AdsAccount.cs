...
public class AdsAccount
{
    ...
    {
        if (response["account_id"] == null) return null;

        var adsAccount = new AdsAccount
        {
            AccountId = response["account_id"], AccountType = response["account_type"],
            AccountStatus = response["account_status"], AccountName = response["account_name"],
