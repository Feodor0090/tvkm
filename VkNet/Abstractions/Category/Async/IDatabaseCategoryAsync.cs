...
public interface IDatabaseCategoryAsync
{
	...
	/// </remarks>
	Task<VkCollection<Country>> GetCountriesAsync(bool? needAll = null, IEnumerable<Iso3166> codes = null,
        int? count = null,
        int? offset = null);
	...
	///     описывающих станции метро, в поле items.
	///     Каждый объект содержит следующие поля:
	///     id (integer) — идентификатор;
	///     name (string) — название;
	///     color (string) — цвет линии.
	...
	/// </remarks>
	Task<VkCollection<MetroStation>> GetMetroStationsAsync(ulong cityId, int? offset = null, int? count = null,
        bool extended = false);
}