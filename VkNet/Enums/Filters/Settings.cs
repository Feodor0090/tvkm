...
{
    private static readonly Dictionary<string, ulong> MaskMap = new()
    {
        {"notify", 1},
        {"friends", 2},
        {"photos", 4},
    ...
    private static Settings GetByName(string name)
    {
        ...
        if (MaskMap.ContainsKey(n))
            return new Settings
            {
                Mask = MaskMap[n], _settings = new List<string> {n}
            };

        if (!Alias.ContainsKey(n)) throw new ArgumentException($"Undefined access setting '{n}'");
    ...
    public static Settings FromJsonString(string val)
    {
        ...
        foreach (var v in vals)
            if (MaskMap.ContainsKey(v.ToLower()))
                res |= GetByName(v);
            else if (Alias.ContainsKey(v.ToLower())) res |= GetByName(v);
    ...
    public static Settings operator |(Settings a, Settings b)
    {
        ...
        {
            if (a.Selected.Contains(s)) continue;
