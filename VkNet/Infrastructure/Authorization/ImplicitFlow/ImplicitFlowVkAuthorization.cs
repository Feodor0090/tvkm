...
public class ImplicitFlowVkAuthorization : IVkAuthorization<ImplicitFlowPageType>
{
    ...
    public ImplicitFlowPageType GetPageType(Uri url)
    {
        ...
        if (parameters.ContainsKey("sid")) return ImplicitFlowPageType.Captcha;

        if (parameters.ContainsKey("__q_hash")) return ImplicitFlowPageType.Consent;

        var original = url.OriginalString;

        if (original.StartsWith("https://oauth.vk.com/blank.html#access_token")) return ImplicitFlowPageType.Result;

        if (original.StartsWith("https://oauth.vk.com/authorize")) return ImplicitFlowPageType.LoginPassword;

        if (original.StartsWith("https://oauth.vk.com/blank.html#error")) return ImplicitFlowPageType.Error;

        if (original.Contains("/login?act=authcheck")) return ImplicitFlowPageType.TwoFactor;
