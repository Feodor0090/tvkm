...
public sealed class AuthorizationFormHtmlParser : IAuthorizationFormHtmlParser
{
    ...
    public async Task<VkHtmlFormResult> GetFormAsync(Uri url)
    {
        var response = await _restClient
            .PostAsync(url, Enumerable.Empty<KeyValuePair<string, string>>(), Encoding.GetEncoding(1251))
            .ConfigureAwait(false);

        if (!response.IsSuccess) throw new VkAuthorizationException(response.Message);
    ...
    private static HtmlNode GetFormNode(HtmlDocument html)
    {
        ...
        if (form == null) throw new VkApiException("Form element not found.");
