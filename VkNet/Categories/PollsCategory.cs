...
public partial class PollsCategory : IPollsCategory
{
    ...
    public Poll GetById(PollsGetByIdParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"is_board", @params.IsBoard}, {"poll_id", @params.PollId}
        });
    }
    ...
    public bool Edit(PollsEditParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"poll_id", @params.PollId}, {"question", @params.Question},
            {"add_answers", Utilities.SerializeToJson(@params.AddAnswers)},
            {"edit_answers", Utilities.SerializeToJson(@params.EditAnswers)},
            {"delete_answers", Utilities.SerializeToJson(@params.DeleteAnswers)}
        });
    }
    ...
    public VkCollection<PollAnswerVoters> GetVoters(PollsGetVotersParams @params)
    {
        ...
        {
            if (answersIds == null) return null;
            ...
            for (var i = 0; i < answersIds.Count; i++)
            {
                ...
                if (i + 1 < answersIds.Count) stringBuilder.Append(',');
            }
        ...
        {
            {"owner_id", @params.OwnerId}, {"is_board", @params.IsBoard}, {"poll_id", @params.PollId},
            {"answer_ids", FormatList(@params.AnswersIds)}, {"friends_only", @params.FriendsOnly},
            {"offset", @params.Offset}, {"count", @params.Count}, {"fields", @params.Fields},
