...
public partial class PhotoCategory : IPhotoCategory
{
    ...
    {
        if (@params.PrivacyView == null) @params.PrivacyView = new List<Privacy>();

        if (@params.PrivacyComment == null) @params.PrivacyComment = new List<Privacy>();

        if (@params.Title.Length < 2)
            throw new VkApiException("Параметр title обязательный, минимальная длина 2 символа");
    ...
    public bool EditAlbum(PhotoEditAlbumParams @params)
    {
        ...
        var parameters = new VkParameters
        {
            {"album_id", @params.AlbumId}, {"title", @params.Title}, {"description", @params.Description},
            {"owner_id", @params.OwnerId}, {"privacy_view", string.Join(",", @params.PrivacyView)},
            {"privacy_comment", string.Join(",", @params.PrivacyComment)},
            {"upload_by_admins_only", @params.UploadByAdminsOnly}, {"comments_disabled", @params.CommentsDisabled}
        };
    ...
    public VkCollection<PhotoAlbum> GetAlbums(PhotoGetAlbumsParams @params, bool skipAuthorization = false)
    {
        return _vk.Call("photos.getAlbums", new VkParameters
            {
                {"owner_id", @params.OwnerId}, {"album_ids", @params.AlbumIds}, {"offset", @params.Offset},
                {"count", @params.Count}, {"need_system", @params.NeedSystem}, {"need_covers", @params.NeedCovers},
                {"photo_sizes", @params.PhotoSizes}
            }, skipAuthorization)
            .ToVkCollectionOf<PhotoAlbum>(x => x);
    }
    ...
    public VkCollection<Photo> Get(PhotoGetParams @params, bool skipAuthorization = false)
    {
        return _vk.Call("photos.get", new VkParameters
            {
                {"owner_id", @params.OwnerId}, {"album_id", @params.AlbumId}, {"photo_ids", @params.PhotoIds},
                {"rev", @params.Reversed}, {"extended", @params.Extended}, {"feed_type", @params.FeedType},
                {"feed", @params.Feed}, {"photo_sizes", @params.PhotoSizes}, {"offset", @params.Offset},
                {"count", @params.Count}
            }, skipAuthorization)
    ...
    /// <inheritdoc />
    public UploadServerInfo GetChatUploadServer(ulong chatId, ulong? cropX = null, ulong? cropY = null,
    ...
    /// <inheritdoc />
    public ReadOnlyCollection<Photo> SaveWallPhoto(string response, ulong? userId, ulong? groupId = null,
        string caption = null)
    {
        var responseJson = response.ToJObject();
        var server = responseJson["server"].ToString();
    ...
    public VkCollection<Photo> Search(PhotoSearchParams @params, bool skipAuthorization = false)
    {
        return _vk.Call("photos.search", new VkParameters
            {
                {"q", @params.Query}, {"lat", @params.Latitude}, {"long", @params.Longitude},
                {"start_time", @params.StartTime}, {"end_time", @params.EndTime}, {"sort", @params.Sort},
                {"offset", @params.Offset}, {"count", @params.Count}, {"radius", @params.Radius}
            }, skipAuthorization)
            .ToVkCollectionOf<Photo>(x => x);
    }
    ...
    public ReadOnlyCollection<Photo> Save(PhotoSaveParams @params)
    {
        ...
        var parameters = new VkParameters
        {
            {"album_id", @params.AlbumId}, {"group_id", @params.GroupId}, {"server", server},
            {"photos_list", photosList}, {"hash", hash}, {"latitude", @params.Latitude},
            {"longitude", @params.Longitude}, {"caption", @params.Caption}
        };
    ...
    public bool Edit(PhotoEditParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"photo_id", @params.PhotoId}, {"caption", @params.Caption},
            {"latitude", @params.Latitude}, {"longitude", @params.Longitude}, {"place_str", @params.PlaceStr},
            {"foursquare_id", @params.FoursquareId}, {"delete_place", @params.DeletePlace},
            {"captcha_sid", @params.CaptchaSid}, {"captcha_key", @params.CaptchaKey}
        });
    }
    ...
    public VkCollection<Photo> GetAll(PhotoGetAllParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"extended", @params.Extended}, {"offset", @params.Offset},
            {"count", @params.Count}, {"photo_sizes", @params.PhotoSizes},
            {"no_service_albums", @params.NoServiceAlbums}, {"need_hidden", @params.NeedHidden},
    ...
    public VkCollection<Photo> GetUserPhotos(PhotoGetUserPhotosParams @params)
    {
        ...
        {
            {"user_id", @params.UserId}, {"count", @params.Count}, {"offset", @params.Offset},
            {"extended", @params.Extended}, {"sort", @params.Sort}
        }).ToVkCollectionOf<Photo>(x => x);
    }
    ...
    public VkCollection<Comment> GetComments(PhotoGetCommentsParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"photo_id", @params.PhotoId}, {"need_likes", @params.NeedLikes},
            {"start_comment_id", @params.StartCommentId}, {"offset", @params.Offset}, {"count", @params.Count},
            {"sort", @params.Sort}, {"access_key", @params.AccessKey}, {"extended", @params.Extended},
            {"fields", @params.Fields}
        }).ToVkCollectionOf<Comment>(x => x);
    }
    ...
    public VkCollection<Comment> GetAllComments(PhotoGetAllCommentsParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"album_id", @params.AlbumId}, {"need_likes", @params.NeedLikes},
    ...
    {
        if (@params.Message.Length > 2048) throw new VkApiException("Максимальное количество символов: 2048.");
    ...
    /// <inheritdoc />
    public bool EditComment(ulong commentId, string message, long? ownerId = null,
    ...
    public ulong PutTag(PhotoPutTagParams @params)
    {
        ...
        {
            {"owner_id", @params.OwnerId}, {"photo_id", @params.PhotoId}, {"user_id", @params.UserId}, {"x", @params.X},
            {"y", @params.Y}, {"x2", @params.X2}, {"y2", @params.Y2}
        });
    }
