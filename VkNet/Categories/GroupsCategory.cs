...
public partial class GroupsCategory : IGroupsCategory
{
        ...
        bool skipAuthorization = false)
    {
        if (userId.HasValue)
        {
            ...
            {
                if (userId.Value < 1) throw new ArgumentException("Идентификатор пользователя должен быть больше 0");
    ...
    /// <inheritdoc />
    public VkCollection<User> GetRequests(long groupId, long? offset = null, long? count = null,
        UsersFields fields = null)
    {
        var parameters = new VkParameters
        {
        ...
        return fields == null
            ? response.ToVkCollectionOf(x => new User
            {
                Id = (long) x
            })
    ...
    public bool SetCallbackSettings(CallbackServerParams @params)
    {
        ...
        if (@params.CallbackSettings != null)
        {
            ...
            {
                if (!(t.GetCustomAttributes(typeof(JsonPropertyAttribute), true).FirstOrDefault() is
                        JsonPropertyAttribute jsonAttr)) continue;

                if (t.GetValue(@params.CallbackSettings, null) is bool propVal) res.Add(jsonAttr.PropertyName, propVal);
            }
        }

        if (@params.ApiVersion != null) res["api_version"] = @params.ApiVersion.Version;
