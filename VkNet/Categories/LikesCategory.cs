...
public partial class LikesCategory : ILikesCategory
{
    ...
    public VkCollection<long> GetList(LikesGetListParams @params, bool skipAuthorization = false)
    {
        ...
        {
            {"type", @params.Type}, {"owner_id", @params.OwnerId}, {"item_id", @params.ItemId},
            {"page_url", @params.PageUrl}, {"filter", @params.Filter}, {"friends_only", @params.FriendsOnly},
            {"extended", @params.Extended}, {"offset", @params.Offset}, {"skip_own", @params.SkipOwn}
        };

        if (@params.FriendsOnly.HasValue && @params.FriendsOnly.Value)
        {
            if (@params.Count <= 100) parameters.Add("count", @params.Count);
        }
        else
        {
            if (@params.Count <= 1000) parameters.Add("count", @params.Count);
        }
    ...
    public long Delete(LikeObjectType type, long itemId, long? ownerId = null, long? captchaSid = null,
