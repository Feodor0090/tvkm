...
public partial class AuthCategory : IAuthCategory
{
    ...
    public bool CheckPhone(string phone, string clientSecret, long? clientId = null, bool? authByPhone = null)
    {
        ...
        {
            {"phone", phone}, {"client_id", clientId}, {"client_secret", clientSecret}, {"auth_by_phone", authByPhone}
        };
    ...
    public string Signup(AuthSignupParams @params)
    {
        ...
        {
            {"first_name", @params.FirstName}, {"last_name", @params.LastName}, {"birthday", @params.Birthday},
            {"client_id", @params.ClientId}, {"client_secret", @params.ClientSecret}, {"phone", @params.Phone},
            {"password", @params.Password}, {"test_mode", @params.TestMode}, {"voice", @params.Voice},
            {"sex", @params.Sex}, {"sid", @params.Sid}
        });
    }
    ...
    public string Restore(string phone, string lastName)
    {
        var response = _vk.Call("auth.restore"
            , new VkParameters
            {
                {"phone", phone}, {"last_name", lastName}
            });
