...
public partial class AccountCategory : IAccountCategory
{
    ...
    public bool RegisterDevice(AccountRegisterDeviceParams @params)
    {
        ...
        return _vk.Call("account.registerDevice",
            new VkParameters
            {
                {"token", @params.Token}, {"device_model", @params.DeviceModel}, {"device_year", @params.DeviceYear},
                {"device_id", @params.DeviceId}, {"system_version", @params.SystemVersion},
                {"settings", JsonConvert.SerializeObject(@params.Settings)}, {"sandbox", @params.Sandbox}
            });
    }
    ...
    {
        if (@params.RelationPartner != null) VkErrors.ThrowIfNumberIsNegative(() => @params.RelationPartner.Id);

        if (@params.Country != null) VkErrors.ThrowIfNumberIsNegative(() => @params.Country.Id);

        if (@params.City != null) VkErrors.ThrowIfNumberIsNegative(() => @params.City.Id);


        var response = _vk.Call("account.saveProfileInfo", new VkParameters
        {
            {"first_name", @params.FirstName}, {"last_name", @params.LastName}, {"maiden_name", @params.MaidenName},
            {"screen_name", @params.ScreenName}, {"sex", @params.Sex}, {"relation", @params.Relation},
            {"relation_partner_id", @params.RelationPartner?.Id}, {"bdate", @params.BirthDate},
            {"bdate_visibility", @params.BirthdayVisibility}, {"home_town", @params.HomeTown},
            {"country_id", @params.Country?.Id}, {"city_id", @params.City?.Id}, {"status", @params.Status},
            {"phone", @params.Phone}
        });

        changeNameRequest = null;

        if (response.ContainsKey("name_request")) changeNameRequest = response["name_request"];
