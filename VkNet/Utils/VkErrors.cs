...
public static class VkErrors
{
    ...
    {
        if (expr.Body is not MemberExpression body) return;
        ...
        where T : struct, IComparable<T>
    {
        if (value.CompareTo(min) < 0 || value.CompareTo(max) > 0)
            throw new ArgumentOutOfRangeException(
                $@"Значение {value} не должно выходить за пределы диапазона [{min}, {max}]");
    }
	...
	public static void ThrowIfNumberIsNegative(Expression<Func<long?>> expr)
    {
        ...
        if (value is < 0) throw new ArgumentException("Отрицательное значение.", name);
    }
    ...
    {
        if (expr == null) throw new ArgumentNullException(nameof(expr), "Выражение не может быть равно null");
	...
	public static JObject IfErrorThrowException(string json)
    {
        ...
        if (!obj.TryGetValue("error", StringComparison.InvariantCulture, out var error)) return obj;

        var vkError = JsonConvert.DeserializeObject<VkError>(error.ToString(), JsonConfigure.JsonSerializerSettings);

        if (vkError == null || vkError.ErrorCode == 0) return obj;
