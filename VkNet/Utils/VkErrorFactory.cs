...
public static class VkErrorFactory
{
	...
	public static VkApiMethodInvokeException Create(VkError error)
    {
        ...
        if (vkApiMethodInvokeExceptions == null) return new VkApiMethodInvokeException(error);

        var exception =
            PerformanceActivator.CreateInstance<VkApiMethodInvokeException>(vkApiMethodInvokeExceptions, Predicate(),
                error);
