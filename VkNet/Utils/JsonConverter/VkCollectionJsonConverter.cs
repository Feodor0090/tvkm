...
public class VkCollectionJsonConverter : Newtonsoft.Json.JsonConverter
{
    ...
    {
        if (!objectType.IsGenericType) throw new TypeAccessException();
        ...
        if (converter != null) collectionField = converter.CollectionField;

        foreach (var item in response[collectionField]) list.Add(item.ToObject(keyType));
