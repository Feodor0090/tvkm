...
public static class ExecuteErrorsHandler
{
	...
	public static ExecuteException GetExecuteExceptions(string response)
    {
        ...
        var executeErrorsResponse =
            JsonConvert.DeserializeObject<ExecuteErrorsResponse>(response, JsonConfigure.JsonSerializerSettings);

        if (executeErrorsResponse?.ExecuteErrors == null) return null;
