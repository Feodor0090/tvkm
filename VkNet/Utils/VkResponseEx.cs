...
public static class VkResponseEx
{
	...
	public static Collection<T>
        ToCollectionOf<T>(this VkResponse response, Func<VkResponse, T> selector) //where T : class
    {
        if (response == null) return new Collection<T>(new List<T>());

        var responseArray = (VkResponseArray) response;

        if (responseArray ==
            null) //TODO: V3022 http://www.viva64.com/en/w/V3022 Expression 'responseArray == null' is always false.
    ...
    {
        if (response == null) return new ReadOnlyCollection<T>(new List<T>());
    ...
    /// <param name="selector"> Функция выборки. </param>
    /// <returns> Коллекция данных только для чтения. </returns>
    public static ReadOnlyCollection<T> ToReadOnlyCollectionOf<T>(this IEnumerable<VkResponse> responses,
    ...
    {
        if (response == null) return new VkCollection<T>(0, Enumerable.Empty<T>());
